...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import blis_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("blis_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "blis_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"blis", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "blis_jll", "src", "blis_jll.jl"))
        @info("blis_ll dev'ed out to /depot/dev/blis_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("6136c539-28a5-5bf0-87cc-b183200dce32"))
platforms = [Pkg.Artifacts.unpack_platform(e, "blis", artifacts_toml) for e in artifacts["blis"]]
...
if best_platform === nothing
    @debug("Unable to load blis; unsupported platform $(triplet(platform_key_abi()))")
else
